...
//UART1 mutex
static T_AolkmeMutexHandle s_uart1Mutex;
//USART1 handle
...
// #ifdef USING_UART_PORT_1
        case UART_NUM_1: {
            ...
            A_Osal_MutexCreate(&s_uart1Mutex);
        }
            break;
// #endif
...
// #ifdef USING_UART_PORT_3
        case UART_NUM_3: {
            ...
            A_Osal_MutexCreate(&s_uart3Mutex);
        }
            break;
// #endif
// #ifdef USING_UART_PORT_4
...
// #ifdef USING_UART_PORT_1
        case UART_NUM_1: {
            A_Osal_MutexLock(s_uart1Mutex);
            readRealSize = RingBuf_Get(&s_uart1ReadRingBuffer, buf, readSize);
            A_Osal_MutexUnlock(s_uart1Mutex);
        }
            break;
// #endif
...
{
    if (Aolkme_CONSOLE_UART_NUM == UART_NUM_1) {
        HAL_UART_Transmit(&s_uart1Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (Aolkme_CONSOLE_UART_NUM == UART_NUM_2) {
