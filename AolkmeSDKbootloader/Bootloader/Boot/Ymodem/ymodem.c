...
static HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout)
{
    ...
    *p_length = 0;
    status = Uart_ReadWithTimeOut(Aolkme_CONSOLE_UART_NUM, &char1, 1, timeout);
    ...
    if (status == HAL_OK) {
        switch (char1) {
            ...
            case EOT:
                break;
            case CA:
                if ((Uart_ReadWithTimeOut(Aolkme_CONSOLE_UART_NUM, &char1, 1, timeout) == HAL_OK) && (char1 == CA)) {
                    packet_size = 2;
                } else {
                    status = HAL_ERROR;
                }
        ...
        *p_data = char1;
        ...
        if (packet_size >= PACKET_SIZE) {
            status = Uart_ReadWithTimeOut(Aolkme_CONSOLE_UART_NUM, &p_data[PACKET_NUMBER_INDEX],
                                ...
                                if (packets_received == 0) {
                                    ...
                                    if (aPacketData[PACKET_DATA_INDEX] != 0) {
                                        ...
                                        if (*p_size > (APPLICATION_FLASH_SIZE + 1)) {
                                            /* End session */
                                            tmp = CA;
                                            Uart_WriteWithTimeOut(Aolkme_CONSOLE_UART_NUM, &tmp, 1, NAK_TIMEOUT);
                                            Uart_WriteWithTimeOut(Aolkme_CONSOLE_UART_NUM, &tmp, 1, NAK_TIMEOUT);
                                            result = COM_LIMIT;
                                        }
...
COM_StatusTypeDef Ymodem_Transmit(uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size)
{
    ...
    while ((!ack_recpt) && (result == COM_OK)) {
        /* Send Packet */
        Uart_WriteWithTimeOut(Aolkme_CONSOLE_UART_NUM, &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE,
                              NAK_TIMEOUT);
        ...
        /* Wait for Ack and 'C' */
        if (Uart_ReadWithTimeOut(Aolkme_CONSOLE_UART_NUM, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) {
            if (a_rx_ctrl[0] == ACK) {
                ack_recpt = 1;
    ...
    while ((size) && (result == COM_OK)) {
        ...
        while ((!ack_recpt) && (result == COM_OK)) {
            ...
            Uart_WriteWithTimeOut(Aolkme_CONSOLE_UART_NUM, &aPacketData[PACKET_START_INDEX],
